...
ap.add_argument("-n", "--sample-size", type=int, default=100, help="maximum sample size for each")
...
lbs = np.array(lbs)
x_train, x_test, y_train, y_test = train_test_split(faces, lbs, test_size=25)
fr.train(x_train, y_train)
...
for face in x_test:
    ...
    (prediction, conf) = fr.predict(face)
    predictions.append(prediction)
    confidence.append(conf)

# show the classification report
print(classification_report(y_test, predictions, target_names=np.unique(labels)))
